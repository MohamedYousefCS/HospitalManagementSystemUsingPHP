<?php
    ...
    if(isset($_POST['acc_login']))
    {
        ...
        //$acc_email = $_POST['acc_ea']
        $acc_pwd = sha1(md5($_POST['acc_pwd']));//double encrypt to increase security
        $stmt=$mysqli->prepare("SELECT acc_number, acc_pwd, acc_id FROM his_acc WHERE  acc_number=? AND acc_pwd=? ");//sql to log in user
        $stmt->bind_param('ss', $acc_number, $acc_pwd);//bind fetched parameters
        $stmt->execute();//execute bind
        $stmt -> bind_result($acc_number, $acc_pwd ,$acc_id);//bind result
        $rs=$stmt->fetch();
        $_SESSION['acc_id'] = $acc_id;
        $_SESSION['acc_number'] = $acc_number;//Assign session to acc_number id
        //$uip=$_SERVER['REMOTE_ADDR'];
        //$ldate=date('d/m/Y h:i:s', time());
        if($rs)
            {//if its sucessfull
                header("location:his_acc_dashboard.php");
            }
